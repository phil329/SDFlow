...
class ConcatNonLinear(nn.Module):
    def __init__(self, dim_in, dim_out, dim_c):
        ...
        self._layer = nn.Linear(dim_in + 1 + dim_c, dim_out*2)
        self._layer2 = nn.Linear(dim_out*2, dim_out)
        self._layer3 = nn.Linear(dim_out, dim_out)
        self.act = nn.ReLU(inplace=True)
        
    def forward(self, context, x):
        if x.dim() == 3:
            context = context.unsqueeze(1).expand(-1, x.size(1), -1)
        out = torch.cat((x, context), dim=2)
        out = self.act(self._layer(out))
        out = self.act(self._layer2(out))
