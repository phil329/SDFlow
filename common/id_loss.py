...
class IDLoss(nn.Module):
    def __init__(self, crop=True, backbone='r50'):
        ...
        if backbone == 'r50':
            self.facenet = iresnet50(pretrained=True).cuda()
        elif backbone == 'r100':
            self.facenet = iresnet100(pretrained=True).cuda()
        else:
            self.facenet = iresnet34(pretrained=True).cuda()
        self.facenet.eval()
        self.crop = crop
    ...
    def extract_features(self, x):
        def resize(x):
            ...
            # from zzhuang_ops.ops import resize
            # return resize(x, 112)
